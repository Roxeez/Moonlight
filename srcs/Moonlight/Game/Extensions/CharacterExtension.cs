...
namespace Moonlight.Game.Extensions
{
    public static class CharacterExtension
    {
        ...
        {
            return character.Map.Monsters.Where(x => x.Vnum == vnum).Where(x => x.Position.IsInRange(character.Position, radius)).OrderBy(x => x.Position.GetDistance(character.Position)).FirstOrDefault();
        }
